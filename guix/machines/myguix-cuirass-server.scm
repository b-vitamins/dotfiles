...
;; Proper import directives for accessing necessary modules
(use-modules (gnu)
             (gnu services avahi)
...
(define %ci-server-block
  (nginx-server-configuration
   (server-name '("ci.myguix.bvits.in"))
   (listen '("443 ssl"))
   (ssl-certificate "/etc/letsencrypt/live/ci.myguix.bvits.in/fullchain.pem")
   (ssl-certificate-key "/etc/letsencrypt/live/ci.myguix.bvits.in/privkey.pem")
	 (locations
    (list (nginx-location-configuration
          ...
          (nginx-location-configuration
           ...
           (body
            (list
             "if ($ssl_client_verify != SUCESS) { return 403; } proxy_pass http://guix-cuirass;")))))))

(define %substitutes-server-block
  (nginx-server-configuration
   (server-name '("substitutes.myguix.bvits.in"))
   (listen '("443 ssl"))
   (ssl-certificate "/etc/letsencrypt/live/ci.myguix.bvits.in/fullchain.pem")
   (ssl-certificate-key "/etc/letsencrypt/live/ci.myguix.bvits.in/privkey.pem")
	 (raw-content '("rewrite ^//(.*)$ /$1 redirect;"))
   (index (list
           "substitutes.index.html"))
   (locations
    (list (nginx-location-configuration
           (uri
            "/signing-key.pub")
...
(define %nginx-redirect-server-block
  (nginx-server-configuration
   (listen '("80"))
   (raw-content (list "return 308 https://$host$request_uri;"))))
...
(operating-system
  (host-name "myguix-cuirass-server")
  (locale "en_US.utf8")
  ...
  (packages (append (list (specification->package "git")
                          ...
                          (specification->package "fontconfig")) %base-packages))
  ...
  (services
   (list (service avahi-service-type)
         ...
         (service nginx-service-type
                  (nginx-configuration
                   (upstream-blocks (list (nginx-upstream-configuration
                                           (name "guix-cuirass")
                                           (servers (list "localhost:8081")))
                                          (nginx-upstream-configuration
                                           (name "guix-publish")
                                           (servers (list "localhost:8080")))))
                   (server-blocks (list %nginx-redirect-server-block
                                        %ci-server-block
         ...
         (service kmscon-service-type
				 ...
				 (service udev-service-type
									(udev-configuration
									 (rules (list (specification->package "lvm2")
				 ...
				 (service special-files-service-type
									`(("/bin/sh" ,(file-append (specification->package "bash") "/bin/sh"))
										...
										("/bin/zsh" ,(file-append (specification->package "zsh") "/bin/zsh"))
										("/bin/perl" ,(file-append (specification->package "perl") "/bin/perl"))
										("/usr/bin/env" ,(file-append coreutils "/bin/env"))))
				 (simple-service 'my-cron-jobs
                         mcron-service-type (list garbage-collector-job))))
  ...
  (file-systems (cons* (file-system
                         ...
                         (device (uuid "87BA-E400" 'fat32))
                         (type "vfat"))
                       (file-system
                         (mount-point "/")
                         (device (uuid "83c5632c-3ad9-4eca-b233-ad9d6c23e5c8" 'ext4))
