...
(define (process-single-file file)
  (if (and (string-suffix? ".scm" file)
      ...
      (begin
        (apply-guix-style file 0)
        1)
      (begin
        (format #t "Skipping non-Scheme file: ~a~%" file)
        0)))
...
(define (process-directory root-dir)
  ...
  (define (handle-error path stat errno result)
    (format #t "Error processing ~a: ~a~%" path (strerror errno))
    result)
...
(define (main args)
  (let* ((opts '((help (single-char #\h))
    ...
    ;; Handle help option
    (when (assoc-ref result 'help)
      (display "Usage: style.scm [OPTION] [FILE-OR-DIRECTORY]...
        ...
        (for-each
         (lambda (target)
           (cond
            ...
            ((eq? 'directory (stat:type (stat target)))
             (let ((dir-count (process-directory target)))
               (set! total-files (+ total-files dir-count))
               (format #t "Processed ~a files in directory: ~a~%" dir-count target)))
            ((eq? 'regular (stat:type (stat target)))
             (let ((file-count (process-single-file target)))
               (set! total-files (+ total-files file-count))))
            (else
             (format #t "Skipping ~a (not a file or directory)~%" target))))
         targets)
        
        ;; Print summary
        (format #t "~%Total: ~a Scheme file(s) formatted with guix style.~%" total-files)))))
